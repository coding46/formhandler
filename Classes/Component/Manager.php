...
class Manager {
	...
	public static function getInstance() {
		if (self::$instance === NULL) {
			self::$instance = new \Tx\Formhandler\Component\Manager();
		}
	...
	*/
	private function loadTypoScriptConfig() {
		if ($this->additionalIncludePaths === NULL) {
			...
			if (!is_array($overrideSettings['settings.'])) {
				$utilityFuncs = \Tx\Formhandler\Utils\UtilityFuncs::getInstance();
			...
			if(TYPO3_MODE === 'BE') {
				$tsconfig = \TYPO3\CMS\Backend\Utility\BackendUtility::getModTSconfig(intval($_GET['id']), '\Tx\Formhandler\mod1');
	...
	public function getComponent($componentName) {
		...
		if (get_class($this) === $componentName) {
			return $this;
		} elseif ('\Tx\Formhandler\Utils\Globals' === $componentName) {
			return \Tx\Formhandler\Utils\Globals::getInstance();
		} elseif ('\Tx\Formhandler\Utils\UtilityFuncs' === $componentName) {
			return \Tx\Formhandler\Utils\UtilityFuncs::getInstance();
		}
