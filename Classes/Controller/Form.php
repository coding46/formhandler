...
class Form extends AbstractController {
		...
		if (is_array($this->settings['finishers.'])) {
			...
			foreach ($this->settings['finishers.'] as $key => $config) {
				if (strpos($key, '.') !== FALSE) {
					...
					if ($className === '\Tx\Formhandler\Finisher_SubmittedOK' && is_array($config['config.'])) {
			...
			$hash = $GLOBALS['TYPO3_DB']->fullQuoteStr($gp['hash'], '\Tx\Formhandler\log');
			if ($tstamp && strpos($hash, ' ') === FALSE) {
				$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('params', '\Tx\Formhandler\log', 'tstamp=' . $tstamp . ' AND unique_hash=' . $hash);
			...
			if ($finisherConf['actions.'][$action . '.'] && !empty($params) && intval($this->utilityFuncs->getSingle($finisherConf['actions.'][$action . '.']['config.'], 'returns')) !== 1) {
				...
				$class = '\Tx\Formhandler\Finisher_SubmittedOK';
				$object = $this->componentManager->getComponent($class);
				...
				if ($idx !== 'disabled') {
					...
					if (is_array($tsConfig) && strlen($className) > 0) {
						if (intval($this->utilityFuncs->getSingle($tsConfig, 'disable')) !== 1) {
							...
							//if the finisher returns HTML (e.g. \Tx\Formhandler\Finisher_SubmittedOK)
	...
	public function validateConfig() {
		$options = array(
			array('to_email', 'sEMAILADMIN', 'finishers', '\Tx\Formhandler\Finisher_Mail'),
			array('to_email', 'sEMAILUSER', 'finishers', '\Tx\Formhandler\Finisher_Mail'),
			array('redirect_page', 'sMISC', 'finishers', '\Tx\Formhandler\Finisher_Redirect'),
			array('required_fields', 'sMISC', 'validators', '\Tx\Formhandler\Validator_Default'),
		);
