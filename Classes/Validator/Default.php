<?php
namespace Tx\Formhandler\Validator;
 ...
 *
 * <code>
 * plugin.Tx_Formhandler.settings.validators.1.class = \Tx\Formhandler\Validator_Default
 *
 * # single error check
 ...
 */
class Default extends AbstractValidator {
		...
		foreach($fieldConf as $key => $fieldSettings) {
			...
			foreach($errorChecks as $check) {
				...
				if(strpos($classNameFix, 'Tx_') === FALSE) {
					$errorCheckObject = $this->componentManager->getComponent('\Tx\Formhandler\ErrorCheck_' . $classNameFix);
					$fullClassName = '\Tx\Formhandler\Errorcheck_' . $classNameFix;
				} else {
