 ...
 *
 * <code>
 * finishers.2.class = \Tx\Formhandler\Finisher_Mail
 ...
 * finishers.2.config.user.attachPDF.class = \Tx\Formhandler\Generator_TcPdf
...
class Mail extends AbstractFinisher {
	...
	protected function initMailer($type) {
		...
		if (is_array($this->settings['mailer.'])) {
			...
			$emailClass = '\Tx\Formhandler\Mailer_HtmlMail';
		}
	...
	protected function parseTemplate($mode, $suffix) {
		...
		if (!$viewClass) {
			$viewClass = '\Tx\Formhandler\View_Mail';
		}

		/* @var $view \Tx\Formhandler\AbstractView */
